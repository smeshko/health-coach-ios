  ...
  if !refresh {
    ...
    if let cached {
      do {
        ...
        // call", which `WeeklyFeature` reads for its freshness label. The stamp stays on the success path.
        var domain = try cached.toDomain()
        domain.cached = true
        return domain
      } catch {
        // A corrupt/format-drifted row is a cache MISS, not a fatal error (Phase 19.2 D2): fall
        // through to the generate path — its `save` overwrites the same-week row (same `isoWeek` PK).
        log.notice(
          "Cached weekly plan failed to decode — treating as a cache miss",
          category: .http,
