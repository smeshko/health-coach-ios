...
enum WeeklyTestSupport {
  /// A test store with the Europe/Sofia frame pinned and an `ImmediateClock` by default (tests that
  /// drive the debounce/cancellation override `\.continuousClock` with a `TestClock`).
  @MainActor
  static func makeStore(
    date: Date,
    _ prepare: @escaping (inout DependencyValues) -> Void
  ) -> TestStoreOf<WeeklyFeature> {
    TestStore(initialState: WeeklyFeature.State()) {
      WeeklyFeature()
    } withDependencies: {
      $0.useEuropeSofia()
