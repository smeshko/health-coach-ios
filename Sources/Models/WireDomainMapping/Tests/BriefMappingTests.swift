...
struct BriefMappingTests {
  @Test func test_dailyBrief_fullyPopulated_carriesEveryField() throws {
    let brief = domainDailyBrief(WireFixtures.dailyBrief())
  ...
  @Test func test_dailyBrief_nullIntakeYesterday_mapsToNil() throws {
    let brief = domainDailyBrief(WireFixtures.dailyBrief(intakeYesterday: nil))
    #expect(brief.intakeYesterday == nil)
  }

  @Test func test_dailyBrief_emptyAlternatives_mapsToEmpty() throws {
    let brief = domainDailyBrief(WireFixtures.dailyBrief(alternatives: []))
  ...
  // strict-decode guard is covered by `WireModels` decode tests (DecodeRoundTripTests). The mappers
  // are now non-throwing and use the DTO's already-domain enum fields directly.

  // MARK: - Weekly plan

  @Test func test_weeklyPlan_fullyPopulated_carriesEveryField() throws {
    let plan = domainWeeklyPlan(WireFixtures.weeklyPlan())
  ...
  @Test func test_weeklyPlan_nilRestDayAndLastWeek_mapToNil() throws {
    let plan = domainWeeklyPlan(WireFixtures.weeklyPlan(restDay: nil, lastWeek: nil))
